...
document.addEventListener('DOMContentLoaded', function () {
	...
	if (subscribeForm) {
		subscribeForm.addEventListener('submit', function (e) {
			...
			// Disable button
			const submitBtn = subscribeForm.querySelector(
				'input[type="submit"]'
			);
			...
			fetch(pfadi_ajax.ajax_url, {
				...
				.catch((error) => {
					...
					messageContainer.innerHTML =
						'Das hat leider nicht geklappt. Bitte versuche es später noch einmal oder wende dich an admin@alvier.ch.';
					messageContainer.className = 'pfadi-message error';
				})
				.finally(() => {
	...
	if (tabs.length > 0) {
		tabs.forEach((tab) => {
			tab.addEventListener('click', function (e) {
				...
				document
					.querySelectorAll('.pfadi-tabs a')
					.forEach((t) => t.classList.remove('active'));
				// Then add to all matching unit
				document
					.querySelectorAll(`.pfadi-tabs a[data-unit="${unit}"]`)
					.forEach((t) => t.classList.add('active'));

				// 2. Update ALL content containers
				const containers = document.querySelectorAll(
					'.pfadi-activities-content'
				);
	...
	document.addEventListener('click', function (e) {
		if (e.target.closest('.pfadi-list-item')) {
			...
			container
				.querySelectorAll('.pfadi-list-item')
				.forEach((i) => i.classList.remove('active'));
			container
				.querySelectorAll('.pfadi-list-content')
				.forEach((c) => c.classList.remove('active'));

			// Add active to clicked item and corresponding content
			item.classList.add('active');
			container
				.querySelector(`.pfadi-list-content[data-id="${id}"]`)
